...
public class LogMessageJsonProviderJsonbTest extends JsonProviderBaseTest {
    @Override
    protected Type type() {
        return Type.JSONB;
    }
    ...
    void testDefaultConfig() throws Exception {
        ...
        final LogMessageJsonProvider provider = new LogMessageJsonProvider(config);

        final ExtLogRecord event = new ExtLogRecord(Level.ALL, "TestMessage {0}", "");
        event.setParameters(new Object[] { "param0" });
        final JsonNode result = getResultAsJsonNode(provider, event);

        String message = result.findValue("LogMessage").asText();
    ...
    void testCustomConfig() throws Exception {
        ...
        Assertions.assertTrue(new LogMessageJsonProvider(config).isEnabled());
    }
}
